...
// Secciones relacionadas con Analíticas (Analytics)
const ANALYTICS_SECTIONS = [
  {
    name: 'Analíticas - Frontend',
    paths: [
  ...
  {
    name: 'Analíticas - Hooks y Utilidades',
    paths: [
...
function analyzeSection(section) {
  ...
  section.paths.forEach(sectionPath => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // Si es migración, filtrar solo las relacionadas con analytics
      if (section.isMigration) {
        const analyticsFiles = files.filter(file => {
          const fileName = path.basename(file).toLowerCase();
          const content = fs.readFileSync(file, 'utf8');
          return fileName.includes('analytics') || 
                 fileName.includes('analitica') ||
                 content.includes('analytics') ||
                 content.includes('CREATE TABLE.*analytics') ||
                 content.includes('CREATE FUNCTION.*analytics');
        });
        currentFiles.push(...analyticsFiles);
      } else {
        ...
        if (refFile.startsWith(relativePath)) {
          ...
          if (section.isMigration) {
            ...
            if (!fileName.includes('analytics') && 
                !fileName.includes('analitica') &&
                !refFile.toLowerCase().includes('analytics')) {
              return; // Skip non-analytics migrations
            }
          }
...
function main() {
  console.log('🔍 Análisis Exhaustivo de Sección de Analíticas (Analytics)');
  console.log(`📌 Commit de referencia: ${REFERENCE_COMMIT}`);
  console.log('='.repeat(60));
  
  const results = ANALYTICS_SECTIONS.map(section => analyzeSection(section));
  ...
  const outputPath = path.join(PROJECT_ROOT, '.analytics-analysis.json');
...
module.exports = { main, ANALYTICS_SECTIONS, REFERENCE_COMMIT };
