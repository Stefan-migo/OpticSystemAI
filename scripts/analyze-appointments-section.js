...
// Secciones relacionadas con Citas y Agendas (Appointments)
const APPOINTMENTS_SECTIONS = [
  {
    name: 'Citas y Agendas - Frontend',
    paths: [
  ...
  {
    name: 'Citas y Agendas - API Routes',
    paths: [
  ...
  {
    name: 'Citas y Agendas - Migraciones',
    paths: [
      'supabase/migrations',
    ],
    isMigration: true,
  ...
  {
    name: 'Citas y Agendas - Hooks y Utilidades',
    paths: [
      'src/hooks/useAppointment',
...
function analyzeSection(section) {
  ...
  section.paths.forEach(sectionPath => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // Si es migración, filtrar solo las relacionadas con appointments
      if (section.isMigration) {
        const appointmentFiles = files.filter(file => {
        ...
        currentFiles.push(...appointmentFiles);
      } else {
        ...
        if (refFile.startsWith(relativePath)) {
          ...
          if (section.isMigration) {
            ...
            if (!fileName.includes('appointment') && 
                !fileName.includes('cita') &&
                !refFile.toLowerCase().includes('appointment')) {
              return; // Skip non-appointment migrations
            }
          }
...
function main() {
  console.log('🔍 Análisis Exhaustivo de Sección de Citas y Agendas (Appointments)');
  console.log(`📌 Commit de referencia: ${REFERENCE_COMMIT}`);
  console.log('='.repeat(60));
  
  const results = APPOINTMENTS_SECTIONS.map(section => analyzeSection(section));
  ...
  const outputPath = path.join(PROJECT_ROOT, '.appointments-analysis.json');
...
module.exports = { main, APPOINTMENTS_SECTIONS, REFERENCE_COMMIT };
