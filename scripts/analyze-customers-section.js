...
// Secciones relacionadas con clientes
const CUSTOMERS_SECTIONS = [
  {
    ...
    paths: [
      ...
      'src/hooks/useCustomers',
    ],
  },
  {
    name: 'Clientes - API Routes',
    paths: [
...
function analyzeSection(section) {
  ...
  section.paths.forEach(sectionPath => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // Si es migración, filtrar solo las relacionadas con clientes
      if (section.isMigration) {
        const customerFiles = files.filter(file => {
          const fileName = path.basename(file).toLowerCase();
          const content = fs.readFileSync(file, 'utf8');
          return fileName.includes('customer') || 
                 fileName.includes('prescription') ||
                 content.includes('customers') ||
                 content.includes('prescriptions') ||
                 content.includes('CREATE TABLE.*customer');
        });
        currentFiles.push(...customerFiles);
      } else {
        ...
        if (refFile.startsWith(relativePath)) {
          ...
          if (section.isMigration) {
            ...
            if (!fileName.includes('customer') && 
                !fileName.includes('prescription') &&
                !refFile.toLowerCase().includes('customer')) {
              return; // Skip non-customer migrations
            }
          }
...
function main() {
  console.log('🔍 Análisis Exhaustivo de Sección de Clientes');
  console.log(`📌 Commit de referencia: ${REFERENCE_COMMIT}`);
  console.log('='.repeat(60));
  
  const results = CUSTOMERS_SECTIONS.map(section => analyzeSection(section));
  ...
  const outputPath = path.join(PROJECT_ROOT, '.customers-analysis.json');
...
module.exports = { main, CUSTOMERS_SECTIONS, REFERENCE_COMMIT };
