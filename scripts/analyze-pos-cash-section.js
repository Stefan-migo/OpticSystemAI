...
// Secciones relacionadas con POS y Caja
const POS_CASH_SECTIONS = [
  {
    name: 'POS/Caja - Frontend',
    paths: [
      'src/app/admin/pos',
      'src/app/admin/cash-register',
      'src/components/admin/pos',
      'src/components/admin/cash',
    ],
  },
  {
    name: 'POS/Caja - API Routes',
    paths: [
      'src/app/api/admin/pos',
      'src/app/api/admin/cash',
      'src/app/api/admin/orders',
      'src/app/api/admin/payments',
    ],
  },
  {
    name: 'POS/Caja - Migraciones',
    paths: [
      'supabase/migrations',
    ],
    isMigration: true,
  ...
  {
    name: 'POS/Caja - Hooks y Utilidades',
    paths: [
      'src/hooks/usePOS',
      'src/hooks/useCashRegister',
      'src/hooks/useOrders',
      'src/lib/pos',
      'src/lib/cash',
    ],
...
function analyzeSection(section) {
  ...
  section.paths.forEach(sectionPath => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // Si es migración, filtrar solo las relacionadas con POS/Caja
      if (section.isMigration) {
        const posCashFiles = files.filter(file => {
          const fileName = path.basename(file).toLowerCase();
          const content = fs.readFileSync(file, 'utf8');
          return fileName.includes('pos') || 
                 ...
                 content.includes('cash_register') ||
                 content.includes('CREATE TABLE.*order') ||
                 content.includes('CREATE TABLE.*payment');
        });
        currentFiles.push(...posCashFiles);
      } else {
        ...
        if (refFile.startsWith(relativePath)) {
          ...
          if (section.isMigration) {
            ...
            if (!fileName.includes('pos') && 
                ...
                !fileName.includes('payment') &&
                !fileName.includes('transaction') &&
                !refFile.toLowerCase().includes('pos') &&
                !refFile.toLowerCase().includes('cash') &&
                !refFile.toLowerCase().includes('order') &&
                !refFile.toLowerCase().includes('payment')) {
              return; // Skip non-POS/Cash migrations
            }
          }
...
function main() {
  console.log('🔍 Análisis Exhaustivo de Sección POS y Caja');
  console.log(`📌 Commit de referencia: ${REFERENCE_COMMIT}`);
  console.log('='.repeat(60));
  
  const results = POS_CASH_SECTIONS.map(section => analyzeSection(section));
  ...
  const outputPath = path.join(PROJECT_ROOT, '.pos-cash-analysis.json');
...
module.exports = { main, POS_CASH_SECTIONS, REFERENCE_COMMIT };
