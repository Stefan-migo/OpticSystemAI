...
// Secciones relacionadas con productos
const PRODUCTS_SECTIONS = [
  {
    name: 'Productos - Frontend',
    paths: [
      'src/app/admin/products',
      'src/components/admin/products',
      'src/components/admin/CreateProductForm',
    ],
  },
  {
    name: 'Productos - API Routes',
    paths: [
  ...
  {
    name: 'Productos - Hooks y Utilidades',
    paths: [
      'src/hooks/useProduct',
      'src/hooks/useProductOptions',
      'src/lib/inventory',
      'src/lib/presbyopia-helpers',
    ],
  },
];
...
function analyzeSection(section) {
  ...
  section.paths.forEach(sectionPath => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // Si es migración, filtrar solo las relacionadas con productos
      if (section.isMigration) {
        const productFiles = files.filter(file => {
          ...
          return fileName.includes('product') || 
                 ...
                 content.includes('CREATE TABLE.*product') ||
                 content.includes('CREATE TABLE.*category');
        });
        currentFiles.push(...productFiles);
      } else {
        ...
        if (refFile.startsWith(relativePath)) {
          ...
          if (section.isMigration) {
            ...
            if (!fileName.includes('product') && 
                ...
                !fileName.includes('inventory') &&
                !fileName.includes('stock') &&
                !refFile.toLowerCase().includes('product') &&
                !refFile.toLowerCase().includes('category')) {
              return; // Skip non-product migrations
            }
          }
...
function main() {
  console.log('🔍 Análisis Exhaustivo de Sección de Productos');
  console.log(`📌 Commit de referencia: ${REFERENCE_COMMIT}`);
  console.log('='.repeat(60));
  
  const results = PRODUCTS_SECTIONS.map(section => analyzeSection(section));
  ...
  const outputPath = path.join(PROJECT_ROOT, '.products-analysis.json');
...
module.exports = { main, PRODUCTS_SECTIONS, REFERENCE_COMMIT };
