...
// Secciones relacionadas con Presupuestos (Quotes)
const QUOTES_SECTIONS = [
  {
    name: 'Presupuestos - Frontend',
    paths: [
  ...
  {
    name: 'Presupuestos - Hooks y Utilidades',
    paths: [
      'src/hooks/useQuote',
      'src/lib/quotes',
    ],
...
function analyzeSection(section) {
  ...
  section.paths.forEach(sectionPath => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // Si es migración, filtrar solo las relacionadas con presupuestos
      if (section.isMigration) {
        const quoteFiles = files.filter(file => {
          const fileName = path.basename(file).toLowerCase();
          const content = fs.readFileSync(file, 'utf8');
          return fileName.includes('quote') || 
                 content.includes('quotes') ||
                 content.includes('CREATE TABLE.*quote');
        });
        currentFiles.push(...quoteFiles);
      } else {
        ...
        if (refFile.startsWith(relativePath)) {
          ...
          if (section.isMigration) {
            ...
            if (!fileName.includes('quote') && 
                !refFile.toLowerCase().includes('quote')) {
              return; // Skip non-quote migrations
            }
          }
...
function main() {
  console.log('🔍 Análisis Exhaustivo de Sección de Presupuestos (Quotes)');
  console.log(`📌 Commit de referencia: ${REFERENCE_COMMIT}`);
  console.log('='.repeat(60));
  
  const results = QUOTES_SECTIONS.map(section => analyzeSection(section));
  ...
  const outputPath = path.join(PROJECT_ROOT, '.quotes-analysis.json');
...
module.exports = { main, QUOTES_SECTIONS, REFERENCE_COMMIT };
