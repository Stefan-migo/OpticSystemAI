...
// Secciones relacionadas con Sistema y Soporte
const SECTIONS = [
  {
  ...
  {
    name: 'Sistema y Soporte - API Routes',
    paths: [
  ...
  {
    name: 'Sistema y Soporte - Migraciones',
    paths: [
      'supabase/migrations',
    ],
    isMigration: true,
...
function analyzeSection(section) {
  ...
  if (section.isFileList && section.specificFiles) {
    // Para secciones con archivos específicos, verificar cada uno
    section.specificFiles.forEach(filePath => {
    ...
    section.paths.forEach(sectionPath => {
      ...
      if (fs.existsSync(fullPath)) {
        ...
        if (stat.isDirectory()) {
          ...
          // Si es migración, filtrar solo las relacionadas con settings o support
          if (section.isMigration) {
            const relevantFiles = files.filter(file => {
              ...
              return fileName.includes('settings') || 
                     ...
                     content.includes('support_tickets') ||
                     content.includes('schedule_settings') ||
                     content.includes('CREATE TABLE.*support') ||
                     content.includes('CREATE TABLE.*settings');
            });
    ...
    section.paths.forEach(sectionPath => {
      ...
      allFilesInRef.forEach(refFile => {
        if (refFile.startsWith(relativePath)) {
          // Si es migración, filtrar solo las relacionadas
          if (section.isMigration) {
            const fileName = path.basename(refFile).toLowerCase();
            if (!fileName.includes('settings') && 
                !fileName.includes('support') &&
                !fileName.includes('system') &&
                !refFile.toLowerCase().includes('settings') &&
                !refFile.toLowerCase().includes('support')) {
...
function main() {
  console.log('🔍 Análisis Exhaustivo de Secciones: Sistema y Soporte');
  ...
  const outputPath = path.join(PROJECT_ROOT, '.system-support-analysis.json');
