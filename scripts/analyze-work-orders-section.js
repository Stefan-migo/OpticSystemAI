...
// Secciones relacionadas con Trabajos (Work Orders)
const WORK_ORDERS_SECTIONS = [
  {
    name: 'Trabajos - Frontend',
    paths: [
      'src/app/admin/work-orders',
      'src/components/admin/work-orders',
      'src/components/admin/CreateWorkOrderForm',
    ],
  },
  {
    name: 'Trabajos - API Routes',
    paths: [
      'src/app/api/admin/work-orders',
      'src/app/api/admin/lab-work-orders',
    ],
  },
  {
    name: 'Trabajos - Migraciones',
    paths: [
      'supabase/migrations',
    ],
    isMigration: true,
  ...
  {
    name: 'Trabajos - Hooks y Utilidades',
    paths: [
      'src/hooks/useWorkOrder',
      'src/lib/work-orders',
      'src/lib/lab',
    ],
  },
...
function analyzeSection(section) {
  ...
  section.paths.forEach(sectionPath => {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // Si es migración, filtrar solo las relacionadas con trabajos
      if (section.isMigration) {
        const workOrderFiles = files.filter(file => {
          const fileName = path.basename(file).toLowerCase();
          const content = fs.readFileSync(file, 'utf8');
          return fileName.includes('work') || 
                 fileName.includes('order') ||
                 fileName.includes('lab') ||
                 content.includes('work_orders') ||
                 content.includes('lab_work_orders') ||
                 content.includes('CREATE TABLE.*work_order') ||
                 content.includes('CREATE TABLE.*lab_work');
        });
        currentFiles.push(...workOrderFiles);
      } else {
      ...
      allFilesInRef.forEach(refFile => {
        if (refFile.startsWith(relativePath)) {
          ...
          if (section.isMigration) {
            ...
            if (!fileName.includes('work') && 
                !fileName.includes('order') &&
                !fileName.includes('lab') &&
                !refFile.toLowerCase().includes('work') &&
                !refFile.toLowerCase().includes('lab')) {
              return; // Skip non-work-order migrations
            }
          }
...
function main() {
  console.log('🔍 Análisis Exhaustivo de Sección de Trabajos (Work Orders)');
  console.log(`📌 Commit de referencia: ${REFERENCE_COMMIT}`);
  console.log('='.repeat(60));
  
  const results = WORK_ORDERS_SECTIONS.map(section => analyzeSection(section));
  ...
  const outputPath = path.join(PROJECT_ROOT, '.work-orders-analysis.json');
...
module.exports = { main, WORK_ORDERS_SECTIONS, REFERENCE_COMMIT };
