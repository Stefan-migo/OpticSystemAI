...
/**
 * Script to apply notification migrations to Supabase database
 * 
 * Usage:
 *   node scripts/apply-notification-migrations.js
 * 
 * Requires:
...
async function applyMigrations() {
  ...
  const migrationPath = path.join(__dirname, '../supabase/migrations/20250129000000_add_optical_notification_types.sql')
  ...
  try {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (errorData.message?.includes('already exists') || 
                ...
                errorData.message?.includes('IF NOT EXISTS')) {
              console.log(`  ⚠️  Skipped (already applied): ${statement.substring(0, 50)}...`)
            } else {
    ...
    if (tablesError && tablesError.code === 'PGRST205') {
      console.error('❌ Table still not found. Please run the SQL manually in Supabase Studio.')
