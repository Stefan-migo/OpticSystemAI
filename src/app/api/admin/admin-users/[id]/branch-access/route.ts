...
export async function GET(
  ...
  try {
    ...
    if (!branchContext.isSuperAdmin) {
      return NextResponse.json({ error: 'Only super admins can view branch access' }, { status: 403 });
    }
    ...
    console.error('Error in GET branch access:', error);
...
export async function POST(
  ...
  try {
    ...
    const branchContext = await getBranchContext(request, user.id);
    if (!branchContext.isSuperAdmin) {
      return NextResponse.json({ error: 'Only super admins can assign branch access' }, { status: 403 });
    }

    // Validate input
    if (branch_id === undefined) {
      return NextResponse.json({ error: 'branch_id is required (use null for super admin)' }, { status: 400 });
    }
    ...
    if (branch_id === null) {
      ...
      if (insertError) {
        console.error('Error creating super admin access:', insertError);
        return NextResponse.json({ error: 'Failed to assign super admin access' }, { status: 500 });
      }
    ...
    if (upsertError) {
      console.error('Error assigning branch access:', upsertError);
      return NextResponse.json({ error: 'Failed to assign branch access' }, { status: 500 });
    }

    return NextResponse.json({ branchAccess });
  } catch (error: any) {
    console.error('Error in POST branch access:', error);
    return NextResponse.json({ error: error.message || 'Internal server error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const branchContext = await getBranchContext(request, user.id);
    if (!branchContext.isSuperAdmin) {
      return NextResponse.json({ error: 'Only super admins can remove branch access' }, { status: 403 });
    }
    ...
    if (deleteError) {
      ...
      return NextResponse.json({ error: 'Failed to remove branch access' }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('Error in DELETE branch access:', error);
    return NextResponse.json({ error: error.message || 'Internal server error' }, { status: 500 });
  }
}
