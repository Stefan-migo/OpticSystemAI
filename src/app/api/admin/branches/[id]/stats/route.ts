...
import { validateBranchAccess, addBranchFilter } from '@/lib/api/branch-middleware'
...
export async function GET(
  ...
  try {
    ...
    const [
      ...
      // Total appointments
      branchFilter(supabase.from('appointments').select('id', { count: 'exact', head: true })),
      
      // Total quotes
      branchFilter(supabase.from('quotes').select('id', { count: 'exact', head: true })),
      
      // Total work orders
      branchFilter(supabase.from('lab_work_orders').select('id', { count: 'exact', head: true })),
      
      // Total orders
      branchFilter(supabase.from('orders').select('id', { count: 'exact', head: true })),
    ...
    const todayRevenueAmount = revenueData.data?.reduce(
    ...
    console.error('Error in GET /api/admin/branches/[id]/stats:', error)
    return NextResponse.json(
