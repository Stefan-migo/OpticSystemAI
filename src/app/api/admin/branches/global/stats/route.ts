...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      // Total work orders
      supabase.from('lab_work_orders').select('id', { count: 'exact', head: true }),
      ...
      Promise.all(
        (branches || []).map(async (branch) => {
          const [appointments, quotes, workOrders, orders, revenue] = await Promise.all([
            supabase
              .from('appointments')
          ...
          const branchRevenue = revenueData.data?.reduce(
    ...
    const globalRevenue = revenueData.data?.reduce(
      (sum: number, order: any) => sum + (parseFloat(order.total_amount) || 0),
      0
    ) || 0
    ...
    console.error('Error in GET /api/admin/branches/global/stats:', error)
    return NextResponse.json(
