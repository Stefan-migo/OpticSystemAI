    ...
    if (error) {
      ...
      if (error.code === 'PGRST205' || error.message?.includes('Could not find the table')) {
        return NextResponse.json({ 
          ...
          message: 'The notification_settings table does not exist. Please run the database migration: 20250129000000_add_optical_notification_types.sql',
          settings: []
        }, { status: 500 });
      }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { notification_type, enabled, priority, notify_all_admins, notify_specific_roles } = body;
    ...
    const { data: updatedSetting, error } = await supabaseServiceRole
      ...
      .upsert({
        ...
        notify_all_admins: notify_all_admins !== undefined ? notify_all_admins : true,
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    for (const update of updates) {
      ...
      if (!notification_type) {
        results.push({ notification_type: null, success: false, error: 'Notification type is required' });
        continue;
      }
    ...
    console.error('Error in notification settings PATCH API:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
