...
export async function GET(request: NextRequest) {
  try {
    ...
    if (quotesWithRelations.length > 0) {
      ...
      // Fetch products
      const productIds = [...new Set(quotesWithRelations.map(q => q.frame_product_id).filter(Boolean))];
      ...
      quotesWithRelations = quotesWithRelations.map(quote => ({
        ...
        prescription: prescriptions?.find(p => p.id === quote.prescription_id) || null,
        frame_product: products?.find(p => p.id === quote.frame_product_id) || null,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: quoteNumber, error: quoteNumberError } = await supabaseServiceRole
