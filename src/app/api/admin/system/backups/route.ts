...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error("Error in backups API:", { error });
    return NextResponse.json(
    ...
    for (const tableName of restoreOrder) {
      ...
      try {
        ...
        if (tableName === "products" && tableData.length > 0) {
          ...
          if (backupProductIds.length > 0) {
            logger.debug(
              ...
              { tableName, count: backupProductIds.length },
            );
            // Delete in batches
            for (let i = 0; i < backupProductIds.length; i += deleteBatchSize) {
              ...
              if (deleteError) {
                logger.error(
                  ...
                  { error: deleteError, tableName, batchSize: batch.length },
                );
                deleteErrors++;
              } else {
          ...
          while (hasMore) {
            ...
            if (fetchError) {
              logger.error(`Error fetching data to delete from ${tableName}:`, {
                error: fetchError,
                tableName,
                offset,
              });
            ...
            for (let i = 0; i < existingData.length; i += deleteBatchSize) {
              ...
              if (ids.length > 0) {
                ...
                if (deleteError) {
                  logger.error(`Error deleting batch from ${tableName}:`, {
                    error: deleteError,
                    tableName,
        ...
        logger.debug(
          ...
          { tableName, totalDeleted, deleteErrors },
        );
          ...
          if (categoryIds.length > 0) {
            ...
            if (invalidCategoryProducts.length > 0) {
              logger.warn(
                ...
                { tableName, invalidCount: invalidCategoryProducts.length },
              );
        ...
        for (let i = 0; i < tableData.length; i += batchSize) {
          ...
          if (cleanBatch.length > 0) {
            ...
            if (insertError) {
              logger.error(`Error inserting batch into ${tableName}:`, {
                error: insertError,
                tableName,
              ...
              if (cleanBatch.length > 1) {
                logger.debug(
                  ...
                  { tableName, batchSize: cleanBatch.length },
                );
                for (const record of cleanBatch) {
                  ...
                  if (singleError) {
                    ...
                    logger.error(`Failed to insert record in ${tableName}:`, {
                      error: singleError,
                      tableName,
                      recordId: record.id,
                    });
