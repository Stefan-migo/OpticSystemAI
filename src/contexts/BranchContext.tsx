  ...
  const fetchBranches = async () => {
    ...
    try {
      ...
      if (data.currentBranch) {
        const branch = data.branches?.find((b: Branch) => b.id === data.currentBranch)
        if (branch) {
        ...
        if (savedBranchId === 'global' && data.isSuperAdmin) {
          setCurrentBranchState(null)
          setIsGlobalView(true)
        } else if (savedBranchId && savedBranchId !== 'global') {
          const branch = data.branches?.find((b: Branch) => b.id === savedBranchId)
          if (branch) {
            setCurrentBranchState(branch)
          } else {
          ...
          const primaryBranch = data.branches?.find((b: Branch) => b.is_primary)
