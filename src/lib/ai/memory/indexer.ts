...
export class MemoryIndexer {
  ...
  async indexProducts(options: IndexingOptions = {}): Promise<IndexingResult> {
    ...
    try {
      ...
      const { data: products, error } = await this.context.supabase
        .from('products')
        .select('id, name, description, short_description, price, status, category_id, benefits, tags')
      ...
      for (let i = 0; i < products.length; i += batchSize) {
        ...
        for (const product of batch) {
          ...
          if (!forceReindex) {
            const hasEmb = await this.semanticMemory.hasEmbedding('product', product.id)
            if (hasEmb) {
        ...
        if (records.length > 0) {
          try {
            ...
            result.errors.push(`Batch ${i / batchSize + 1} failed: ${err.message}`)
          }
        }
      }
  ...
  async indexCustomers(options: IndexingOptions = {}): Promise<IndexingResult> {
    ...
    try {
      const { data: customers, error } = await this.context.supabase
        .from('customers')
        .select('id, email, first_name, last_name, phone, membership_tier, total_orders, total_spent')
      ...
      for (let i = 0; i < customers.length; i += batchSize) {
        ...
        for (const customer of batch) {
          if (!forceReindex) {
            const hasEmb = await this.semanticMemory.hasEmbedding('customer', customer.id)
            if (hasEmb) {
              result.indexed++
              continue
            }
      ...
      console.log(`Customers indexed: ${result.indexed}/${result.totalRecords}`)
      return result
    } catch (error: any) {
      result.errors.push(`Indexing failed: ${error.message}`)
      return result
  ...
  async indexOrders(options: IndexingOptions = {}): Promise<IndexingResult> {
    ...
    try {
      const { data: orders, error } = await this.context.supabase
        .from('orders')
        .select(`
      ...
      for (let i = 0; i < orders.length; i += batchSize) {
        ...
        for (const order of batch) {
          if (!forceReindex) {
            const hasEmb = await this.semanticMemory.hasEmbedding('order', order.id)
            if (hasEmb) {
              result.indexed++
              continue
            }
  ...
  async indexChatSession(sessionId: string): Promise<IndexingResult> {
    ...
    try {
      ...
      const records: EmbeddingRecord[] = messages.map(msg => ({
        sourceType: 'chat_message',
        sourceId: msg.id,
        content: msg.content,
   ...
   * Index categories
   */
  async indexCategories(options: IndexingOptions = {}): Promise<IndexingResult> {
    ...
    try {
      ...
      for (const category of categories) {
        if (!forceReindex) {
          const hasEmb = await this.semanticMemory.hasEmbedding('category', category.id)
          if (hasEmb) {
            result.indexed++
            continue
          }
      ...
      console.log(`Categories indexed: ${result.indexed}/${result.totalRecords}`)
      return result
    } catch (error: any) {
      result.errors.push(`Indexing failed: ${error.message}`)
      return result
   ...
   */
  async indexAll(options: IndexingOptions = {}): Promise<Record<string, IndexingResult>> {
