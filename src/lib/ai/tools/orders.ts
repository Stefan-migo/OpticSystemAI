...
export const orderTools: ToolDefinition[] = [
  {
    name: 'getOrders',
    description: 'Get list of orders with optional filters for status and payment status.',
    category: 'orders',
    parameters: {
      type: 'object',
      properties: {
    ...
    execute: async (params, context): Promise<ToolResult> => {
      try {
        ...
        const { data, error, count } = await supabase
          .from('orders')
          .select('id, order_number, email, total_amount, created_at', { count: 'exact' })
          .eq('status', 'pending')
  ...
  },
  {
    ...
    execute: async (params, context): Promise<ToolResult> => {
      try {
        ...
        return { success: false, error: error.message || 'Failed to get order stats' }
      }
    }
  }
]
